...
class MarketEnvironment:
    """Market environment with better microstructure simulation"""
    ...
    def run_simulation(self, n_steps=None):
        """Simulation with market microstructure features"""
        ...
        for t in range(n_steps):
            ...
            # Market makers place orders with context
            for agent in self.agents:
        ...
        # Post-processing
        self.post_process_trades()

    def post_process_trades(self, window_size=10, price_move_threshold=0.0015):
        """Toxicity labelling with multiple criteria"""
    ...
    def record_order_data(self, order, timestamp):
        """Record data about each order for ML training with features"""
        # Find the agent type
    ...
    def plot_results(self, save_dir="plots"):
        ...
        fig.suptitle('Market Simulation Results', fontsize=16, fontweight='bold')
        ...
        # 2. Market maker spread dynamics
        mm = next((agent for agent in self.agents if isinstance(agent, MarketMaker)), None)
        if mm:
            ...
            axes[0,1].set_title('Market Maker Spread Adjustment')
        ...
        if agent_returns:
            ...
            colours = ['green', 'red', 'blue'][:len(agent_types)]
            axes[2,0].bar(x_pos, agent_means, yerr=agent_stds, capsize=5, color=colours, alpha=0.7)
            axes[2,0].set_title('Agent Performance')
...
def run_simulation(save_data=True, output_dir="market_data"):
    """Run simulation optimised for ML training"""
    ...
    print("Running simulation for ML training")
    ...
    mm = next((agent for agent in market.agents if isinstance(agent, MarketMaker)), None)
    if mm:
    ...
    print(f"{'='*60}")
    print("Data files contain features for:")
    print("• Volatility-adjusted spread modelling")
    print("• Adverse selection detection")
    print("• Order flow analysis")
    print("• Market microstructure patterns")
    print("• Multi-criteria toxicity labelling")
