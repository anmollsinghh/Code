...
class AdvancedToxicityDetector:
    ...
    def train_enhanced_ensemble(self, X_scaled, hyperparameters=None):
        ...
        print("Training Isolation Forest variants...")
        contamination_rates = [0.025, 0.05, 0.1, 0.15]
        for i, contamination in enumerate(contamination_rates):
            try:
                print(f"  Training Isolation Forest {i+1}/{len(contamination_rates)} (contamination={contamination})")
                iso_forest = IsolationForest(
                    contamination=contamination,
                    n_estimators=min(200, hyperparameters.get('n_estimators', 200)),
                    random_state=42 + i,
                    bootstrap=True,
                ...
                print(f"  ✓ Isolation Forest {contamination} completed")
            except Exception as e:
                print(f"  ✗ Failed to train Isolation Forest with contamination {contamination}: {e}")

        # LOF variants
        print("Training LOF variants...")
        neighbor_counts = [5, 10, 20]
        for i, neighbors in enumerate(neighbor_counts):
            try:
                print(f"  Training LOF {i+1}/{len(neighbor_counts)} (neighbors={neighbors})")
                lof = LocalOutlierFactor(
                ...
                print(f"  ✓ LOF {neighbors} completed")
            except Exception as e:
                print(f"  ✗ Failed to train LOF with {neighbors} neighbors: {e}")

        # One-Class SVM variants (with size limits)
        print("Training SVM variants...")
        if n_samples <= 5000:  # Only train SVM on smaller datasets
            svm_configs = [
                {'kernel': 'rbf', 'gamma': 'scale', 'nu': 0.05, 'max_iter': 1000},
                {'kernel': 'rbf', 'gamma': 'scale', 'nu': 0.1, 'max_iter': 1000},
                {'kernel': 'linear', 'nu': 0.1, 'max_iter': 1000}
            ]
            
            for i, config in enumerate(svm_configs):
                try:
                    print(f"  Training SVM {i+1}/{len(svm_configs)} ({config['kernel']} kernel)")
                    svm = OneClassSVM(**config)
                    svm.fit(X_scaled)
                    detectors[f'svm_{config["kernel"]}_{config["nu"]}'] = svm
                    print(f"  ✓ SVM {config['kernel']} completed")
                except Exception as e:
                    print(f"  ✗ Failed to train SVM {config['kernel']}: {e}")
                    continue
        else:
            print(f"  Skipping SVM training - dataset too large ({n_samples} samples)")

        # Gaussian Mixture Models
        print("Training GMM variants...")
        ...
        for i, n_components in enumerate(component_counts):
            try:
                ...
                print(f"  Training GMM {i+1}/{len(component_counts)} (components={n_comp})")
                gmm = GaussianMixture(
                    n_components=n_comp,
                    random_state=42,
                ...
                print(f"  ✓ GMM {n_comp} completed")
            except Exception as e:
                print(f"  ✗ Failed to train GMM with {n_comp} components: {e}")

        # K-means based detectors
        print("Training K-means variants...")
        ...
        for i, n_clusters in enumerate(cluster_counts):
            try:
                ...
                print(f"  Training K-means {i+1}/{len(cluster_counts)} (clusters={n_clust})")
                kmeans = KMeans(n_clusters=n_clust, random_state=42, n_init=10, max_iter=300)
                cluster_labels = kmeans.fit_predict(X_scaled)
                ...
                print(f"  ✓ K-means {n_clust} completed")
            except Exception as e:
                print(f"  ✗ Failed to train K-means with {n_clust} clusters: {e}")
        ...
        print(f"\n✓ Training completed: {len(detectors)} detectors trained successfully")
