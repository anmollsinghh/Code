...
import random
import os
from academic_validation_analysis import validate_existing_simulation
...
class MarketEnvironment:
    ...
    def plot_additional_analytics(self, save_dir="plots"):
        ...
        # 6. Spread vs Toxicity Correlation
        if not market_df.empty and not trades_df.empty:
            # Calculate rolling toxicity rate - handle duplicate timestamps
            window = 50
            
            # Aggregate trades by timestamp first to handle duplicates
            trades_by_timestamp = trades_df.groupby('timestamp')['is_toxic'].mean().reset_index()
            trades_by_timestamp['rolling_toxicity'] = trades_by_timestamp['is_toxic'].rolling(window=window, min_periods=1).mean() * 100
            
            # Create a mapping dictionary
            toxicity_map = dict(zip(trades_by_timestamp['timestamp'], trades_by_timestamp['rolling_toxicity']))
...
if __name__ == "__main__":
   ...
   validator, results = validate_existing_simulation(market)
