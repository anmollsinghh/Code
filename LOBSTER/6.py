...
class EnhancedMarketDataFeatureEngineer:
    ...
    def preprocess_lobster_data(self, orders_df, lob_df):
        ...
        # Process orders data
        if 'Size' in orders_df.columns:
            orders_df['quantity'] = pd.to_numeric(orders_df['Size'], errors='coerce').fillna(0)
        ...
        for i in range(1, 11):
            ...
            if ask_price_col in lob_df.columns:
                lob_df[ask_price_col] = pd.to_numeric(lob_df[ask_price_col], errors='coerce').fillna(0)
            if ask_size_col in lob_df.columns:
                lob_df[ask_size_col] = pd.to_numeric(lob_df[ask_size_col], errors='coerce').fillna(0)
            if bid_price_col in lob_df.columns:
                lob_df[bid_price_col] = pd.to_numeric(lob_df[bid_price_col], errors='coerce').fillna(0)
            if bid_size_col in lob_df.columns:
                lob_df[bid_size_col] = pd.to_numeric(lob_df[bid_size_col], errors='coerce').fillna(0)
        ...
        if len(lob_df) > 0:
            if 'Ask_Price_1' in lob_df.columns and 'Bid_Price_1' in lob_df.columns:
                ...
                # Calculate order book imbalance (first 5 levels)
                total_bid_size = pd.Series(0.0, index=lob_df.index)
                total_ask_size = pd.Series(0.0, index=lob_df.index)
                ...
                for i in range(1, 6):
                    ...
                    if bid_size_col in lob_df.columns:
                        total_bid_size += lob_df[bid_size_col]
                    if ask_size_col in lob_df.columns:
                        total_ask_size += lob_df[ask_size_col]
                
                # Calculate imbalance with safety check
                total_volume = total_bid_size + total_ask_size
                lob_df['order_book_imbalance'] = np.where(
                    total_volume > 0,
                    (total_bid_size - total_ask_size) / total_volume,
                    0
                )
        ...
        if 'mid_price' in orders_df.columns:
            # Only calculate for valid mid prices
            valid_prices = orders_df['mid_price'] > 0
            
            if valid_prices.any():
                # Calculate volatility
                orders_df['volatility'] = 0.0
                price_series = orders_df.loc[valid_prices, 'mid_price']
                if len(price_series) > 1:
                    rolling_std = price_series.rolling(20, min_periods=1).std().fillna(0)
                    orders_df.loc[valid_prices, 'volatility'] = rolling_std
                
                # Calculate momentum
                orders_df['momentum'] = 0.0
                if len(price_series) > 5:
                    momentum = price_series.pct_change(5).fillna(0)
                    orders_df.loc[valid_prices, 'momentum'] = momentum
            else:
                orders_df['volatility'] = 0.0
                orders_df['momentum'] = 0.0
        
        print(f"Preprocessing completed:")
        print(f"  Orders with valid data: {len(orders_df)}")
        print(f"  LOB snapshots processed: {len(lob_df)}")
        if 'mid_price' in orders_df.columns:
            valid_mid_prices = (orders_df['mid_price'] > 0).sum()
            print(f"  Valid mid prices: {valid_mid_prices}")
        
        return orders_df, lob_df

    def _extract_enhanced_lob_features(self, lob_df, orders_df):
        """Extract enhanced LOB features from LOBSTER format"""
    ...
    def load_market_data(self, orderbook_file, message_file):
        """Load lobster market data from your specific files"""
        print(f"Loading lobster market data...")
        
        # Load orderbook snapshots
        print(f"Loading orderbook data from {orderbook_file}...")
        
        # Define headers for orderbook data (40 columns: 20 price-size pairs)
        lob_headers = []
        for i in range(1, 11):
...
def main_enhanced_training_pipeline(orderbook_file, message_file, n_trials=30):
    ...
    print("Advanced Feature Engineering & Ensemble Methods - LOBSTER Data")
    print("="*80)
    
    # Step 1: Load lobster market data
    print("\n1. LOADING LOBSTER MARKET DATA")
    print("-" * 40)
    
    feature_engineer = EnhancedMarketDataFeatureEngineer()
    orders_df, lob_df, trades_df = feature_engineer.load_market_data(orderbook_file, message_file)
    
    # Step 1.2: Validate data
    print("\n1.2. VALIDATING LOBSTER DATA")
    print("-" * 40)
    
    feature_engineer.validate_lobster_data(orders_df, lob_df)
    
    # Step 1.5: Preprocess lobster data
    print("\n1.5. PREPROCESSING LOBSTER DATA")
    print("-" * 40)
    
    orders_df, lob_df = feature_engineer.preprocess_lobster_data(orders_df, lob_df)
...
if __name__ == "__main__":
    try:
        # Specify your lobster data files
        orderbook_file = "AMZN_Orderbook.csv"  # Your 40-column orderbook file
        message_file = "AMZN_Order_Message.csv"      # Your message file with headers
        
        print("Starting enhanced toxicity detection model training with LOBSTER data...")
        print(f"\nUsing files:")
        print(f"  Orderbook file: {orderbook_file}")
        ...
        print("\n" + "="*80)
        print("SUCCESS: Enhanced training completed with LOBSTER data!")
        print("="*80)
        
    except FileNotFoundError as e:
        print(f"Data Error: {e}")
        print("\nPlease ensure:")
        print("1. Your LOBSTER orderbook CSV file exists (40 columns)")
        print("2. Your LOBSTER message CSV file exists with headers:")
        print("   Time,Type,Order ID,Size,Price,Direction")
